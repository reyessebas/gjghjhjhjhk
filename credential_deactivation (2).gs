 ...
 * 4. Creates a PDF.
 * 5. Emails the PDF to the addresses in column H (Recipients).
...
function handleDeactivationEdit(e) {
  ...
  try {
    ...
    const recipients = clean_(values[7]);
    ...
    if (!recipients) {
      throw new Error(
        'Recipients is blank. Add at least one email address in column H.'
      );
    }
